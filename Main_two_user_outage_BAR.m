...
for rveci = 1: length(Rvec)
    ...
    for cti = 1 : ct
        ...
        if power_search_t(cti)<0 | power_search_t(cti) == inf
    ...
    temp1 = (power_conv_t == P_max);
    outage_conv(rveci) = sum(temp1)/ct;
    temp1 = (power_fix_t == P_max);
    outage_fix(rveci) = sum(temp1)/ct;
    temp1 = (optimal_samples_t == P_max);
    outage_ce(rveci) = sum(temp1)/ct;
    temp1 = (power_search_t == P_max);
    outage_search(rveci) = sum(temp1)/ct;
    temp1 = (power_search_t == P_max);
    power_ratio(rveci) = mean(power_ratioaly_t); 
    outage_ratio(rveci) = sum(temp1)/ct;

end

%bar3(outage_conv, outage_fix)
